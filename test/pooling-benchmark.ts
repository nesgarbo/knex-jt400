...
const baseConfig = {
  ...
  port: process.env.IBMI_PORT ? parseInt(process.env.IBMI_PORT) : 8471,
...
async function concurrentBenchmark(
  ...
  console.log(
    `   Concurrency: ${concurrency}, Iterations per worker: ${iterations}`,
  );
...
async function main() {
  ...
  try {
    ...
    concResults.push(
      await concurrentBenchmark("Knex Pooling", knexPoolConfig, 10, 50),
    );
    // Uncomment after implementing ODBC pooling fix:
    ...
    seqResults.forEach((r) => {
      console.log(
        `  ${r.name.padEnd(20)} | Avg: ${r.avg.toFixed(2)}ms | p95: ${r.p95.toFixed(2)}ms`,
      );
    });
    ...
    concResults.forEach((r) => {
      console.log(
        `  ${r.name.padEnd(20)} | Throughput: ${r.throughput.toFixed(2)} q/sec`,
      );
    });
  } catch (error) {
