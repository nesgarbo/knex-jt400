...
describe("Regression Tests", () => {
  describe("WITH UR optimization hints", () => {
      ...
      const query = knex
        ...
        .whereExists(
          knex
            .select("*")
            .from("othertable")
            .where("othertable.id", "=", knex.raw("test.id")),
        );
      testSql(
        query,
        "select * from test where exists (select * from othertable where othertable.id = test.id)",
      );
    });
    ...
    it("should NOT add WITH UR to subqueries in NOT EXISTS clauses", () => {
      ...
      testSql(
        query,
        "select * from test where not exists (select * from othertable where othertable.id = test.id)",
      );
    });
    ...
    it("should NOT add WITH UR to subqueries in WHERE clauses", () => {
      ...
      testSql(
        query,
        "select * from test where id = (select id from othertable where name = 'John')",
      );
    });

    it("should NOT add WITH UR to CTEs (Common Table Expressions)", () => {
      const query = knex
        .with("t", knex.select("x", "y").from("test"))
        .select("x", "y")
        .from("t");
    ...
    it("should NOT add WITH UR to complex nested queries", () => {
      ...
      testSql(
        query,
        "select * from t where t.x = 1 and (t.y in (2, 3) or (t.y = 4 and t.z = 5))",
      );
    });
  });
  ...
  describe("INSERT FINAL TABLE wrapping", () => {
    ...
    it("should handle INSERT with single column", () => {
      ...
      testSql(
        query,
        "select IDENTITY_VAL_LOCAL() from FINAL TABLE(insert into users (name) values ('test'))",
      );
    });
  });
  ...
  describe("UPDATE optimization hints", () => {
    ...
    it("should NOT add optimization hints to UPDATE with subqueries", () => {
      ...
      testSql(
        query,
        "update test set x = 10, y = 20 where exists (select * from othertable where othertable.id = test.id)",
      );
    });
  });
  ...
  describe("UPDATE returning transaction-based approach", () => {
    ...
    it("should compile UPDATE returning with complex WHERE clause", () => {
      ...
      testSql(
        query,
        "update users set status = 'active' where age > 18 and verified = true",
      );
    });

    it("should handle UPDATE returning with no WHERE clause", () => {
      const query = knex("settings")
