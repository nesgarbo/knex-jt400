...
describe("Select Statements", () => {
  ...
  it("supports select with whereIn", () => {
    const query = knex.select(["x", "y"]).from("test").whereIn("x", [1, 2, 3]);
  ...
  it("supports select with whereNull", () => {
    const query = knex.select(["x", "y"]).from("test").whereNull("x");

    testSql(query, "select x, y from test where x is null");
  });

  it("supports select with whereNotNull", () => {
    const query = knex.select(["x", "y"]).from("test").whereNotNull("x");
  ...
  it("supports select with whereRaw", () => {
    const query = knex.select(["x", "y"]).from("test").whereRaw("x = 1");
  ...
  it("supports select with a fromRaw and bindings", () => {
    const query = knex
      .select(["x", "y"])
      .fromRaw(knex.raw("test where x = ?", [1]));
  ...
  it("supports complex where clauses", () => {
    const query = knex
      ...
      .where("t.x", 1)
      .where((q1) => {
        q1.whereIn("t.y", [2, 3]).orWhere((q2) => {
          q2.where("t.y", 4).andWhere("t.z", 5);
        });
      });
