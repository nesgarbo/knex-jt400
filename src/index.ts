...
class DB2Client extends knex.Client {
  ...
  constructor(config: Knex.Config<DB2Config>) {
    ...
    this.valueForUndefined = config.useNullAsDefault
      ? null
      : this.raw("DEFAULT");
  ...
  async acquireRawConnection() {
    ...
    const opts: Record<string, any> = {
      ...
      port: connectionConfig.port ?? 50000,
      ...(connectionConfig.connectionStringParams || {}), // p.ej. naming, libraries, translate, etc.
    };

    // Usamos SIEMPRE el pool de jt400; para “sin pool” podemos dejar min=1 desde Knex
    const connection: Connection = jt400Pool(opts);
    return connection;
  }
  ...
  createMigrationRunner(
    config?: Partial<
      import("./migrations/ibmi-migration-runner.js").IBMiMigrationConfig
    >,
  ) {
